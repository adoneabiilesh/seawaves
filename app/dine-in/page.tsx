...
import { Loader2, ShoppingBag, Search, SlidersHorizontal, ChevronLeft, Menu, Star, AlertCircle, DollarSign } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useToast } from '@/components/ui/use-toast';
import { Product, CartItem, Category, TableSession } from '@/types';
...
export default function DineInPage() {
    ...
    const { cart, setCart, addons, setIsCartOpen, restaurantId, setTableNumber } = useApp();
    ...
    const validateSession = async () => {
        ...
        try {
            // Lookup active session by table number (session token optional for permanent QR codes)
    ...
    const placeOrder = async () => {
        ...
        try {
            // Get session ID from state or localStorage
            const tableSessionId = session?.id || localStorage.getItem('dineInSessionId');

            // Simplified table fetch
            ...
            const { data: order, error: orderError } = await supabase
                ...
                .insert({
                    ...
                    tableSessionId: tableSessionId || null, // Link to session
                    status: 'pending',
                    total: totalPrice,
            ...
            <div className="flex flex-col items-center justify-center min-h-screen bg-[#FFFFF0] p-6 text-center">
                <div className="bg-white border-2 border-black rounded-2xl p-8 max-w-md shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <div className="space-y-3">
                        <Button onClick={() => validateSession()} className="w-full" variant="outline">
                            Try Again
                        </Button>
                        <p className="text-sm text-gray-500">
                            Ask a staff member to open your table session
                        </p>
                    </div>
                </div>
            </div>
