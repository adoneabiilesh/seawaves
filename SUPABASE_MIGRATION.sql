...
-- Add menuItemId to OrderItem (if not exists) for better joins
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
CREATE INDEX IF NOT EXISTS "TableSession_isActive_idx" 
  ON "TableSession"("isActive");
  
CREATE INDEX IF NOT EXISTS "Rating_menuItemId_idx" 
  ON "Rating"("menuItemId");
  
CREATE INDEX IF NOT EXISTS "Rating_orderId_idx" 
  ON "Rating"("orderId");
...
BEGIN
  ...
  RAISE NOTICE 'Created tables: TableSession, Rating, Review';
  RAISE NOTICE 'Added location fields to Restaurant';
  RAISE NOTICE 'Added tableSessionId to Order';
END $$;
